...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Include the login backend logic
    include_once("../backend/auth_logic.php");
    
    // Call the login handler
    $result = handleLogin($conn, $_POST);
    
    // Perform redirect based on result
    $status = isset($result['status']) ? urlencode($result['status']) : "";
    $type = isset($result['type']) ? urlencode($result['type']) : "";
    $redirect = $result['redirect'];
    
    $url = $redirect . (strpos($redirect, '?') === false ? '?' : '&');
    header("Location: " . $url . "status=$status&type=$type");
    exit();
}
}
...
<!-- Mirrored from hms.cognisun.net/oreo/html/light/sign-in.html by HTTrack Website Copier/3.x [XR&CO'2014], Sun, 06 Oct 2024 17:19:27 GMT -->
<head>
<meta charset="utf-8">
...
<!-- Mirrored from hms.cognisun.net/oreo/html/light/sign-in.html by HTTrack Website Copier/3.x [XR&CO'2014], Sun, 06 Oct 2024 17:19:28 GMT -->
</html>
